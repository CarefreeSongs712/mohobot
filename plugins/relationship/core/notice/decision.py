"""通知决策 — 移植自 astrbot_plugin_relationship (core/notice/decision.py)。

管理员变动/禁言/被踢/被拉群 的自动处理(通知 + 自动退群/拉黑)。
"""
...
class NoticeDecision:
    def __init__(self, ws_server, bot_id: str, message: NoticeMessage, config: PluginConfig):
    ...
    async def decide(self) -> NoticeResult:
        ...
        if self.msg.notice_type == "group_admin":
            await self._handle_admin_change(result)
        elif self.msg.notice_type == "group_ban":
            await self._handle_ban(result)
        elif self.msg.notice_type == "group_decrease" and self.msg.sub_type == "kick_me":
            await self._handle_kicked(result)
        elif self.msg.notice_type == "group_increase" and self.msg.sub_type == "invite":
    ...
    async def _get_operator_name(self) -> str:
        if self._operator_name is None:
            self._operator_name = await get_nickname(
                self.ws_server, self.bot_id,
                group_id=self.msg.group_id, user_id=self.msg.operator_id,
            )
    ...
    async def _check_blacklist(self, result: NoticeResult, group_name: str, gid: int) -> bool:
    ...
    async def _check_capacity(self, result: NoticeResult) -> bool:
        ...
        if isinstance(group_list, list) and len(group_list) > max_cap:
            result.admin_reply += (
                f"\n我已经加了{len(group_list)}个群（超过了{max_cap}个），这群我退了"
            )
            result.operator_reply = (
                f"我最多只能加{max_cap}个群，现在已经加了{len(group_list)}个群，请不要拉我进群了"
            )
            result.leave_group = True
            return True
        return False
    ...
    async def _check_mutual_blacklist(self, result: NoticeResult, gid: int) -> bool:
        ...
        members = await api_call(
            self.ws_server, self.bot_id, "get_group_member_list", {"group_id": int(gid)}
        )
        ...
        member_name = await get_nickname(
            self.ws_server, self.bot_id, group_id=gid, user_id=member_id,
        )
        result.admin_reply += (
