...
def main() -> None:
    con = sqlite3.connect("data/song_knowledge/knowledge_db.db")
    names = [r[0] for r in con.execute(NAMES_SQL).fetchall()]
    print(f"待重抓(空或无换行): {len(names)}", flush=True)

    cj = json.load(open("data/song_knowledge/anubis_cookies.json", encoding="utf-8"))
    UA = "Mozilla/5.0"
    ...
    for name in names:
        ...
        url = ("https://vcpedia.cn/api.php?action=query&prop=revisions&rvprop=content"
        ...
        con.execute(
            ...
            (lyrics, intro,
             credits.get("uploader", ""), credits.get("singers", ""),
             credits.get("lyricist", ""), credits.get("composer", ""),
             credits.get("arranger", ""), credits.get("mixer", ""),
             credits.get("tuner", ""), credits.get("mastering", ""),
             credits.get("pv", ""), credits.get("illustrator", ""),
             credits.get("year", "") or None, name))
        con.commit()
        updated += 1
        time.sleep(0.8)
